...
const _sfc_main$r = /* @__PURE__ */ defineComponent({
  ...
  __ssrInlineRender: true,
  setup(__props) {
    const VPLocalSearchBox = defineAsyncComponent(() => import("./VPLocalSearchBox.Dv-CGwTZ.js"));
...
function qidongmulugundong() {
  ...
  let dingshiqi;
  const shezhimulubiaojidonghua = (lianjie) => {
...
function qidongshoujimuluzhuangtai() {
  ...
  let zuihougengxin = 0;
  let donghuazhen = 0;
  const huoqubiaotiliebiao = () => Array.from(
    document.querySelectorAll(".VPDoc :where(h1,h2,h3,h4,h5,h6)")
  ).filter((biaoti) => {
    var _a;
    return biaoti.id && ((_a = biaoti.textContent) == null ? void 0 : _a.trim());
  });
  const huoqudangqianbiaoti = () => {
    ...
    const shifoudibu = Math.abs(window.scrollY + window.innerHeight - document.body.offsetHeight) < 1;
    if (shifoudibu) {
      return biaotiliebiao[biaotiliebiao.length - 1];
    }
    return biaotiliebiao.reduce((dangqian, biaoti) => {
  ...
  const gengxinanniu = (biaoti) => {
    ...
    const morenwenben = anniu.dataset.morenwenben || ((_a = wenbenjiedian.textContent) == null ? void 0 : _a.trim()) || "";
    anniu.dataset.morenwenben = morenwenben;
    wenbenjiedian.textContent = biaoti ? `${((_b = biaoti.textContent) == null ? void 0 : _b.trim()) || morenwenben} ` : `${morenwenben} `;
    anniu.classList.toggle("youxianzai", Boolean(biaoti));
  };
  const gengxinlianjie = (biaoti) => {
    const dangqianlianjie = biaoti ? `#${biaoti.id}` : "";
    document.querySelectorAll(".VPLocalNavOutlineDropdown .outline-link").forEach((lianjie) => {
...
const RawTheme = {
  ...
  enhanceApp(shangxiawen) {
    var _a;
    (_a = theme.enhanceApp) == null ? void 0 : _a.call(theme, shangxiawen);
    qidongzhutiguodu();
    qidongsousuoxiaoguo();
    qidongmulugundong();
